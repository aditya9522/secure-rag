...
function renderPage(view: View, identity: string, organization: OrganizationOption, canManageMembers: boolean, previewRole: Role, actualRole: Role, theme: Theme, onNavigate: (view: View) => void, onToggleTheme: () => void, onRoleChange: (role: Role) => void, onProfileUpdated: (profile: { user: AuthResponse["user"]; current_organization: OrganizationSummary; organizations: OrganizationSummary[] }) => void) {
  if (view === "overview") return <OverviewPage identity={identity} canManageMembers={canManageMembers} actualRole={actualRole} onNavigate={onNavigate} />;
  if (view === "chat") return <ChatPage key={organization.id} organization={organization.name} identity={identity} onNavigate={onNavigate} />;
  if (view === "knowledge") return <KnowledgePage canManage={canManageMembers} />;
...
export function AuthenticatedWorkspace({ session, onSessionUpdated, onSignedOut }: { session: AuthResponse; onSessionUpdated: (session: AuthResponse) => void; onSignedOut: () => void }) {
  ...
  const canManageMembers = actualRole !== "User";
  ...
  const effectiveView: View = (!canManageMembers && (view === "members" || view === "audit")) || (actualRole === "Organization" && view === "audit") ? "chat" : view;
  ...
  async function changeOrganization(next: OrganizationOption) {
    if (!next.id) return;
    setWorkspaceNotice("");
    try { const updated = await switchOrganization(next.id); setPreviewRole(sessionRole(updated)); onSessionUpdated(updated); }
