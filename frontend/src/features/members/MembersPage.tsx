...
import { getClassificationGuide, groupGuidance, roleGuidance } from "../../data/accessGuidance";
import toast from "react-hot-toast";

const classifications: Classification[] = ["public", "internal", "confidential", "restricted"];
const MemberAccessChart = lazy(() => import("./MemberAccessChart").then((module) => ({ default: module.MemberAccessChart })));
const pageSize = 100;
...
export function MembersPage({ canGrantAdministrator }: { canGrantAdministrator: boolean }) {
  ...
  const loadMembers = async (append = false) => {
    ...
    try {
      const page = await listMembers(pageSize, append ? members.length : 0);
      setMembers((current) => append ? [...current, ...page] : page);
  ...
  useEffect(() => {
    if (!editing) return;
    requestAnimationFrame(() => editingFormRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }));
  }, [editing]);
  ...
  function openEditor(member: MemberSummary) {
    if (member.role === "owner") {
      setNotice("Owner access is protected and cannot be changed from this screen.");
      setError("");
      return;
    }
    setInviteOpen(false);
  ...
  return <div className="page-stack">
    ...
    {inviteOpen && <section className="panel inline-form"><form onSubmit={submitInvite}><h3>Invite a member</h3><p className="form-help">The invitee receives a one-time link. Set the smallest role, clearance, and group scope needed for their work.</p><label>Full name<input value={name} onChange={(event) => setName(event.target.value)} required minLength={2} /></label><label>Email address<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /><small>Use the person’s work email so the invite can be verified safely.</small></label><label>Role<select value={role} onChange={(event) => setRole(event.target.value as "admin" | "member")}><option value="member">Member</option>{canGrantAdministrator && <option value="admin">Administrator</option>}</select></label><div className="field-guide"><strong>{role === "admin" ? "Administrator access" : "Member access"}</strong><span>{roleGuidance[role]}</span></div><label>Classification clearance<select value={classification} onChange={(event) => setClassification(event.target.value as Classification)}>{classifications.map((value) => <option key={value}>{value}</option>)}</select></label><div className="field-guide"><strong>{selectedClassification.label} clearance</strong><span>{selectedClassification.summary}</span><small>Example: {selectedClassification.example}</small></div><label className="field-wide">Groups<input value={groups} onChange={(event) => setGroups(event.target.value)} placeholder="engineering, support" /><small>{groupGuidance.useCase} Keep access least-privileged. {groupGuidance.empty}</small></label><div><button className="button button-primary" disabled={busy}>{busy ? "Creating…" : "Create invitation"}</button><button type="button" className="button button-secondary" onClick={() => { setInviteOpen(false); resetForm(); }}>Cancel</button></div></form></section>}
    {editing && <section className="panel inline-form"><form ref={editingFormRef} onSubmit={saveMember}><h3>Update member access</h3><p className="form-help">Changes apply to future source retrieval and chat requests for this member.</p><div className="edit-member-summary field-wide"><span className="profile-avatar table-avatar">{editing.full_name.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span><div><strong>{editing.full_name}</strong><small>{editing.email}</small></div><span className={`status-pill ${editing.status === "active" ? "status-active" : "status-pending"}`}><span />{editing.status}</span></div><label>Role<select autoFocus value={editRole} onChange={(event) => setEditRole(event.target.value as MemberRole)}><option value="member">Member</option>{(canGrantAdministrator || editing.role === "admin") && <option value="admin">Administrator</option>}<option value="suspended">Suspended</option></select></label><label>Classification clearance<select value={classification} onChange={(event) => setClassification(event.target.value as Classification)}>{classifications.map((value) => <option key={value}>{value}</option>)}</select></label><div className="field-guide field-wide"><strong>{selectedClassification.label} clearance</strong><span>{selectedClassification.summary}</span><p>{selectedClassification.useCase}</p></div><label className="field-wide">Groups<input value={groups} onChange={(event) => setGroups(event.target.value)} placeholder="engineering, support" /><small>{groupGuidance.useCase}</small></label><div><button className="button button-primary" disabled={busy}>{busy ? "Saving…" : "Save access"}</button><button type="button" className="button button-secondary" onClick={() => setEditing(null)}>Cancel</button></div></form></section>}
    <div className="role-banner"><div className="role-banner-icon"><Users size={18} /></div><div><strong>Role-based access control</strong><p>Every question and source is evaluated against the member’s current organization, groups, and classification clearance.</p></div><button className="text-button" onClick={() => setNotice("Access is enforced by the API and retrieval policy on every request.")}>View policy <ArrowUpRight size={14} /></button></div><Suspense fallback={<section className="audit-chart-card audit-chart-loading" aria-label="Loading member access chart">Loading member access chart…</section>}><MemberAccessChart members={members} /></Suspense>
    <section className="panel"><div className="table-toolbar"><h3 className="table-title">Organization members <span>{members.length || "—"}</span></h3><div className="search-field compact"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search members" aria-label="Search members" /></div></div><div className="member-table"><div className="table-row table-head"><span>Member</span><span>Role</span><span>Groups</span><span>Status</span><span /></div>{visibleMembers.length ? visibleMembers.map((member) => <div className="table-row" key={member.user_id}><div className="member-cell"><span className="profile-avatar table-avatar">{member.full_name.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span><span><strong>{member.full_name}</strong><small>{member.email}</small></span></div><span className="role-text">{member.role}</span><span className="org-count">{member.groups.join(", ") || "No groups"}</span><span className={`status-pill ${member.status === "active" ? "status-active" : "status-pending"}`}><span />{member.status}</span><button className="icon-button subtle member-action-button" type="button" onClick={(event) => { event.stopPropagation(); openEditor(member); }} aria-label={member.role === "owner" ? `${member.full_name} owner access` : `Manage ${member.full_name}`} title={member.role === "owner" ? "Owner access cannot be changed" : "Manage access"}><MoreHorizontal size={17} /></button></div>) : <div className="empty-state"><Users size={20} /><strong>{search ? "No matching members" : "No member data available"}</strong><span>{search ? "Try a different name, email, or role." : "Member data will appear here when the organization is ready."}</span></div>}{hasMore && <button className="button button-secondary load-more-button" type="button" disabled={loadingMore} onClick={() => void loadMembers(true)}>{loadingMore ? "Loading…" : "Load more members"}</button>}</div></section>
