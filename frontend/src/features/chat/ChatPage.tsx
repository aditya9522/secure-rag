...
export function ChatPage({ organization, identity, onNavigate }: { organization: string; identity: string; onNavigate: (view: "settings") => void }) {
  ...
  const citedSources = ([...messages].reverse().find((message) => message.role === "assistant" && message.citations?.length)?.citations ?? []).filter((source, index, sources) => sources.findIndex((item) => item.document_title === source.document_title) === index);
  useEffect(() => {
    if (!menuOpen) return;
  ...
  useEffect(() => { if (!import.meta.env.VITE_API_BASE_URL) return; void listConversations().then((items) => { setRemoteConversations(items); if (items[0]) setConversationId(items[0].id); }).catch((requestError) => setError(requestError instanceof Error ? requestError.message : "Could not load conversation history.")); }, []);
  useEffect(() => {
  ...
  return <div className={`chat-layout ${contextOpen ? "" : "chat-layout-context-closed"}`}><aside className="conversation-rail"><div className="conversation-head"><div><div className="eyebrow accent-eyebrow">Assistant</div><h2>Conversations</h2></div><button className="icon-button subtle" aria-label="New conversation" onClick={newConversation}><Plus size={17} /></button></div><button className="new-chat-button" onClick={newConversation}><Plus size={16} /> New conversation</button><div className="conversation-list">{conversations.map((conversation, index) => <button key={conversation.id ?? conversation.title} className={`conversation-item ${activeConversation === index ? "conversation-item-active" : ""}`} onClick={() => { if (conversation.id) selectConversation(conversation.id, index); }}><span className="conversation-icon"><MessageSquare size={15} /></span><span className="conversation-info"><strong>{conversation.title}</strong><small>{conversation.preview}</small></span><time>{conversation.time}</time></button>)}{!conversations.length && <div className="conversation-empty">No saved conversations yet.</div>}</div><div className="rail-footer"><div className="usage-label"><span>Conversation history</span><strong>{remoteConversations.length}</strong></div><small>History is isolated to the current organization and signed-in user.</small></div></aside>
